...
# id_simulator = bullet.connect(bullet.DIRECT)  # or bullet.DIRECT for non-graphical version
bullet.setTimeStep(delta_t)

# Switch between URDF with/without FIXED joints
with_fixed_joints = True
# with_fixed_joints = False

if with_fixed_joints:
    id_revolute_joints = [0, 3]
    id_robot = bullet.loadURDF("../pybullet_data/TwoJointRobot_w_fixedJoints.urdf",
                               robot_base,
                               robot_orientation,
                               useFixedBase=True)
else:
    id_revolute_joints = [0, 1]
    id_robot = bullet.loadURDF("../pybullet_data/TwoJointRobot_wo_fixedJoints.urdf",
                               robot_base,
                               robot_orientation,
                               useFixedBase=True)
...
if plot:
    ...
    plt.savefig(f"torque_control-{'fixed' if with_fixed_joints else 'wo_fixed'}_joints.png")
